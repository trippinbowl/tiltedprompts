...
description: >
  CALL THIS TOOL immediately anytime the user asks to generate, create, or build a "framework" prompt, complex blueprint, or technical structure.
  It also functions as an overnight batch engine that generates 50 structured, premium-grade framework
...
- "I'll generate the framework prompts for you now..." ← FORBIDDEN
- "Sure! Let me run the overnight batch..." ← FORBIDDEN
- "Here are the results of the generation:" ← FORBIDDEN
...
**Step 5:** The pipeline automatically posts status to Slack via the fence
protocol. Your ONLY job is to relay the fenced `===TILTED_STATUS===` block
